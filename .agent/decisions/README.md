...
- **0055** — Deploy via **per-user systemd on the shared box**; the `docker-compose.yml` + headless IB-Gateway container is **dropped, not deferred**. The close-capture is a per-close one-shot (no resident process to containerise), the gateway logs in headless without a container, and the systemd stack is already green. Deploy source of truth: `scripts/systemd/README.md`; readiness probe: `scripts/eod_healthcheck.py`.
- **0056** — `residual_observations` contract (as-of, append-only) banks the realized-attribution **residual** time series + its named Taylor terms + as-of unmodeled-exposure covariates (skew/regime/vol-of-vol; liquidity/slippage reserved); missing covariate = `None`, never a fabricated zero. The walk-forward residual regression (`scipy.lstsq`) is **gated** on a configured min out-of-sample depth (default floor 45 banked days for 6 factors): below it returns `GATED` not a coefficient. Estimator proven on synthetic planted data; **live path gated** because canonical depth is shallow (3 trade-dates, no fills/book partitions). Honors §6 by gating the verdict, not refusing to build.
- **0059** — Re-enable **full-membership** constituent option capture (index options AND all ~40-50 constituents, every close) for **maximum option-surface data** (owner directive; live entitlement verified). Resurrects the ADR-0045 lane scoped to the *whole* as-of membership (config: `capture_constituents` default true, `constituent_top_n` default null=all), restores `ConstituentCaptureOutcome`/`constituent_capture_outcomes`, regen `universe` hash. **Amends 0051's capture-scope clause only — the full-membership realized-vol ρ̄ diagnostic is KEPT unchanged** (additive capture, not a ρ̄ input). *(amends 0051; re-scopes 0045)*
- **0060** — **Scope-aware** EOD QC severity: the **index stays strict** (CRITICAL pages + blocks banking), a **constituent** CRITICAL-gate failure is **downgraded to notice** (`is_index` flag + `_scope_critical`), so expected single-name illiquidity stops paging every night and a constituent-only-failing date banks. Banking predicate moves to "escalation != page". No served marks, no config-hash move. *(QC consequence of 0059; same family as 0052)*
- **0061** — **Support-aware calendar-arbitrage materiality.** Calendar no-arb is on **total variance** `w(k,T)=IV²·T` (non-decreasing in `T`; falling IV is fine). A gross inversion pages CRITICAL only when `k` is **within the observed strike support of both slices** (intersection of their observed log-moneyness envelopes); a gross inversion in the **extrapolation region** is downgraded to a **WARNING** (don't page on arbitrage between fabricated marks). Threads per-slice observed `[k_min,k_max]` from `SliceFit.raw_points` → `CalendarSlice` → `CalendarViolation`; config `calendar_support_aware`/`calendar_support_epsilon` in hashed `qc.yaml`, default-new + reversible. Stops the false SX5E nightly page; keeps paging on real within-support defects. No served marks change. Option-1 (calendar-coupled SVI / monotone repair) remains the deferred default-off fix. *(orthogonal + additive on 0060; lineage of 0052)*
