...
| Plan of record (domain + strategy) | `TARGET.md` | **The single source — what we build, why, in what order, and the domain authority on any formula/contract conflict** (§0 frozen scope + universe model, §7 ordered sequence). `BIG_PICTURE.md` (repo root) is the system overview; `.agent/open-questions.md` is the canonical pending-decision register. `TARGET.md` absorbed the retired blueprint — it is the sole domain authority (ADR 0011, amended). The `documentation/` tree is gone; git history is the archive. For course pedagogy (formulas, strategy rationale), see `ThomasHossen/MM_options_trading.md`. | `TARGET.md` |
