...
The loader is the only place that knows the on-disk shape of the config. It
turns lists into the tuples the frozen dataclasses expect and raises a clear
error naming the missing section rather than a raw ``KeyError`` from deep inside.
"""
