...
``AppContext`` holds the resolved :class:`storage.ParquetStore`, the configs directory,
and the default underlying/window the routers fall back to. It is injectable so tests
can point it at a ``tmp_path`` store with fixture data; in production ``build()`` walks
...
# Fallbacks when no config names a default. AAPL is the underlying our fixtures use.
...
class ContextError(Exception):
    ...
    def __init__(self, start: Path) -> None:
        self.start = start
        super().__init__(
            f"could not locate workspace root from {start}: no '{_ROOT_MARKER}' in the parent chain"
        )
