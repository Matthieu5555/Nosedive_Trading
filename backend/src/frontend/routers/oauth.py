...
reports that the Saxo broker backend is not configured in the flat backend — the token
exchange and live-session injection need ``packages/infra-saxo``, which arrives with the
restructure. Everything that can be verified without a live broker (state lifecycle, URL
construction, CSRF rejection) is real here; the rest fails closed with a typed payload.
"""
...
def saxo_callback(
    ...
    # State is valid; the live exchange needs the Saxo backend (not in the flat backend yet).
    return JSONResponse(
        {
...
def saxo_status() -> JSONResponse:
    """Report the Saxo session status (not configured in the flat backend)."""
