...
a hand when something is wrong.

The fastest path — run the canonical end-of-day sequence from one place:

    from orchestration import EodStages, run_end_of_day, build_metrics
...
* **jobs** — universe refresh, live collection, incremental analytics, EOD
  reconciliation, each a function of injected dependencies with a structured,
...
* **metrics** — the five well-labeled prometheus metrics (event rate, stale ratio,
