...
A job here is one unit of operable work: universe refresh, live collection,
incremental analytics, end-of-day reconciliation, and QC. Every job takes its
dependencies (the store, the config, a clock, the metric bundle) as parameters rather
than constructing them, so a test drives a job with fakes and an injected clock and
nothing reaches a real broker or a wall clock. Every job emits a structured log line
bound to a ``correlation_id``, and that same id is threaded from the collector session
through the analytics run, so a single trace resolves a session to the jobs it fed —
the actor already binds the id onto its own log lines, and these jobs propagate it.
...
from actor import ActorOutputs, persist_outputs, run_analytics
...
from connectivity import Clock, SessionSupervisor
...
from snapshots import SnapshotBatch, build_snapshots
...
def refresh_universe(
    *,
    ...
    Workstream B owns the resolution of a broker chain into masters; this job
...
def collect_live(
    *,
    ...
    """Drive B's collector over a session and record its event-rate metric.

    Constructs the :class:`collectors.MarketDataCollector` with the injected store,
    universe, and clock, runs one collection session over the supervised stream (B
    owns the loss-aware persistence; this job orchestrates it), and bumps the
    ``events_collected_total`` counter by the observations captured, labeled by
    underlying. The ``session_id`` is the correlation handle: it is stable across
    restarts (so a restart resumes the same session, not a fresh one) and it is the
    id the downstream analytics run carries, which is what links a session to the
    jobs it fed. Returns the collector's daily summary.
    """
    log = _LOGGER.bind(
        correlation_id=correlation_id,
        job="collection",
    ...
    summary = collector.collect(supervisor, subscribe=subscribe)
...
def _record_collection_metrics(
    ...
    Reads the day's observations back off the raw layer and counts them per
    underlying, so the ``events_collected_total`` counter is labeled by underlying
    rather than lumped into one opaque total — the well-labeled half of the metrics
    gotcha. Gap meta-events are not observations and are not counted.
    """
