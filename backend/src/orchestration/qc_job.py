...
The QC plane (``src/qc``) is a pure check library: it does not read a clock, persist
...
def run_qc(
    *,
    ...
    collector_summary: CollectorSummary | None,
    trade_date: date,
    run_id: str,
    run_ts: datetime,
    correlation_id: str,
    ...
    rolls the report, writes the rows, and computes escalation. ``run_id`` and
    ``run_ts`` are injected and stamped on every row, so the QC output reproduces in
    replay. Persisting is idempotent on the result key (replace-semantics). Returns the
    report and the one escalation signal an alert layer thresholds on.
    """
    log = _LOGGER.bind(
    ...
    log.info(
        ...
        triage_headlines=[row.headline for row in table.rows],
    )
    return QcJobResult(
        correlation_id=correlation_id,
