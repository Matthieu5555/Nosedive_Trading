...
This is the validation plane for the platform: a library of pure, named checks that
each take a producing workstream's output and return a stamped
:class:`contracts.QcResult`. The design rule is specificity. A QC failure must name
the exact maturity, quote, underlying, or solver that broke — a generic red banner
is the precise failure mode these checks exist to prevent — so every failing result
carries the offending object's name in its context payload.

The fastest path to a daily verdict:

    from config.loader import ...                      # load PlatformConfig
    from qc import thresholds_from_config, build_report, triage_table, escalation_level
    from qc import check_collector_continuity, ...     # run the checks you have inputs for
    ...
    table = triage_table(report)            # worst-first, names the offenders
