...
``config.QcThresholdConfig`` is small and A-owned (this workstream does not edit it):
...
*not* read by any QC check — quote staleness is gated upstream in C's snapshot builder
(``snapshots.check_quote_age``), and a snapshot that failed it is already non-usable
before it reaches a check here; the property is kept for completeness and traceability.
The remaining checks need their own cut-offs (a max gap count, a max parity residual,
a max solver non-convergence ratio, and so on). Rather than edit A's contract to bolt
those on, we wrap it.
...
class QcThresholds:
    ...
    ``config`` is A's ``QcThresholdConfig`` (read-only here); its three fields gate
