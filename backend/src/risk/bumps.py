...
reasons that have nothing to do with economics (``tasks/04-risk-engine.md`` calls
this out, and ``tasks/TESTING.md`` makes "Greeks and the scenario engine draw the
bump from one shared versioned source" a required test). Versioning the spec means
a change to a bump is a deliberate, reviewable bump of ``BUMP_VERSION``, not a
silent edit buried in one module.
