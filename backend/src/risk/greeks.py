...
The production path takes Greeks straight from C's frozen pricer — analytic for
European, the lattice for American — so D never re-derives a sensitivity the
keystone already exposes. :func:`central_difference_greeks` is the independent
cross-check: it differences the *pricer's price* using the shared
:data:`risk.bumps.DEFAULT_BUMPS`, and the test that analytic and central-difference
Greeks agree is what catches a sign or unit error even where analytic Greeks are
used (``tasks/TESTING.md``). Both this and the scenario engine's local
approximation draw their bump from one versioned source, so they cannot diverge.

Monetization conventions (consistent with the pricer's per-unit cash Greeks in
``pricing.engine``; D scales them by contract multiplier and signed held quantity):
...
def position_risk(
    *,
    ...
    contracts, mirroring :func:`pricing.price`.
    """
...
def net_lots(lines: Iterable[PositionRisk]) -> list[PositionRisk]:
    ...
    A's :class:`contracts.Position` carries a ``source`` field, so the same
    ``(portfolio_id, contract_key)`` legitimately appears as several lots — a broker
    holding plus a hypothetical overlay, say. But A's *derived* contracts have no lot
    dimension: ``RiskAggregate`` is net-per-group and ``ScenarioResult`` is keyed by
    ``(portfolio, scenario, contract)``. So the line is the contract, by construction.
    Netting here sums the signed quantities (the only thing that differs between lots),
    keeps the shared market state, and returns one line per ``(portfolio_id,
    contract_key)`` sorted by contract key — making every line-level and scenario-cell
    ordering a pure function of the input *set*, which the byte-identical replay in
    Workstream E depends on (see ADR 0006).
