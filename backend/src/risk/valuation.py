"""The valuation input D prices a position against, and the adapter to the pricer.

D's pure core does not reach into C's six contracts directly; it takes one typed,
resolved market state per contract — :class:`ContractValuationInput` — and turns it
into the pricer's :class:`pricing.PricingState`. Keeping D's input a single owned
dataclass (rather than a tuple of C's snapshot + forward + surface objects) is what
keeps the risk functions pure and testable from fixtures, and confines the join
against C's contracts to one thin assembly step.

The state is built through :func:`pricing.from_spot`, so the forward is *derived*
from spot and carry and the pricer's ``forward == spot * exp(carry * T)`` invariant
holds by construction — D can never hand the pricer an internally inconsistent
state. Spot and carry are therefore the stored anchors; the forward is a derived
view (:attr:`ContractValuationInput.forward`).
"""
...
from pricing import PricingState, from_spot

# Confidence labels carried from C's quote QC (ADR 0004): a position on a contract
# C flagged low-confidence is still priced, but the label rides through to the line
# so it can be surfaced, never silently dropped.
CONFIDENCE_OK = "ok"
CONFIDENCE_LOW = "low"
...
class ContractValuationInput:
    ...
    instrument and drive monetization; ``confidence`` carries C's quote-QC verdict
...
def pricing_state_for(valuation: ContractValuationInput) -> PricingState:
    ...
    Uses :func:`pricing.from_spot` so the forward is derived consistently and the
    pricer's forward-consistency invariant cannot be violated by D.
    """
    return from_spot(
        spot=valuation.spot,
