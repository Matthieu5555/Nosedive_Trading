...
in :func:`qc.robust_z_score`; both compute the same median/MAD statistic, and each plane's
test pins it against an independent hand computation. A future shared ``utils`` could
unify them, but the validation plane stays importable on its own until then.
"""
