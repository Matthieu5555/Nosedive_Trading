...
job-result obligations. The four headline cases are:
...
* ``test_correlation_id_links_collector_session_to_analytics`` — drive a session then
  an analytics run under one correlation id and assert the same id appears on both the
  collector's and the analytics job's structured log lines (the trace resolves end to
  end).
* ``test_*_metric_increments_on_*`` — a forward failure bumps the forward-failure
  counter; a non-converging solve bumps the solver-failure counter; collected events
  bump the event counter — each on the right event.
...
from orchestration.run_state import OUTCOME_FAILED, OUTCOME_OK
from qc import thresholds_from_config
from risk import BrokerGreeks
...
# --------------------------------------------------------------------------- #
# Collector scaffolding (mirrors test_collectors): fake broker over a 2-leg chain #
# --------------------------------------------------------------------------- #
...
def _live_universe() -> UniverseService:
    resolved = resolve_chain(_CHAIN_ROWS)
    return UniverseService([c.instrument for c in resolved], TRADE_DATE)


def _broker_tick(sequence: int, value: float) -> BrokerTick:
    return BrokerTick(
        broker_contract_id="c1", field_name="bid", value=value, sequence=sequence,
...
def _eod_stages(
    ...
    *,
    ...
    from orchestration.jobs import CollectionResult
...
def test_correlation_id_links_collector_session_to_analytics(tmp_path: Path) -> None:
    # Drive a real collector session, then an analytics run, under one correlation id;
    # assert the same id is on a collector-job log line and an analytics-job line.
    chain = get_fixture("synthetic_known_answer")
    events, instruments, masters = _chain_inputs(chain)
    store = ParquetStore(tmp_path)
    _seed_raw_layer(store, events)
    ...
    # The trace also carries the collection side under the same id.


def test_collection_and_analytics_share_one_correlation_id_end_to_end(tmp_path: Path) -> None:
    # A live collection followed by the analytics it feeds: one id spans both log lines.
    store = ParquetStore(tmp_path)
    universe = _live_universe()
    correlation_id = "corr-session-42"
    ...
    try:
        ...
        supervisor = SessionSupervisor(
            FakeBrokerSession(script=[_broker_tick(1, 10.0), _broker_tick(2, 11.0)]),
            client_id=2000, clock=clock,
        )
        supervisor.connect()
        collect_live(
            store=store, universe=universe, supervisor=supervisor, subscribe=("c1",),
            session_id=correlation_id, trade_date=TRADE_DATE, clock=ManualClock(start=_T0),
            correlation_id=correlation_id,
        )
        run_incremental_analytics(
            store=store, config=_config(), config_hash=CONFIG_HASH, positions=[],
            instruments=[c.instrument for c in resolve_chain(_CHAIN_ROWS)],
            masters=[_master(c.instrument) for c in resolve_chain(_CHAIN_ROWS)],
            trade_date=TRADE_DATE, as_of=AS_OF, calc_ts=CALC_TS,
            clock=ManualClock(start=CALC_TS), correlation_id=correlation_id, persist=False,
        )
    finally:
        structlog.reset_defaults()

    collection_lines = [
        r for r in capture.records if r.get("event") == "orchestration.collection.done"
    ]
    analytics_lines = [
        r for r in capture.records if r.get("event") == "orchestration.analytics.start"
    ]
    assert collection_lines and analytics_lines
    assert collection_lines[0]["correlation_id"] == correlation_id
    assert analytics_lines[0]["correlation_id"] == correlation_id
...
def test_collected_events_metric_increments_per_underlying(tmp_path: Path) -> None:
    # A real collection persists two AAPL observations; the counter reads 2 for AAPL.
    store = ParquetStore(tmp_path)
    metrics = build_metrics()
    clock = ManualClock(start=_T0)
    supervisor = SessionSupervisor(
        FakeBrokerSession(script=[_broker_tick(1, 10.0), _broker_tick(2, 11.0)]),
        client_id=2001, clock=clock,
    )
    supervisor.connect()
    result = collect_live(
        store=store, universe=_live_universe(), supervisor=supervisor, subscribe=("c1",),
        session_id="sess-metric", trade_date=TRADE_DATE, clock=ManualClock(start=_T0),
        correlation_id="corr-metric", metrics=metrics,
    )
    assert result.summary.event_count == 2  # independent: two ticks, both observations
    assert sample_value(metrics.registry, "events_collected_total", {"underlying": "AAPL"}) == 2.0
...
def _risk_line(contract_key: str, *, quantity: float = 1.0):  # type: ignore[no-untyped-def]
    ...
    from risk import ContractValuationInput, position_risk
