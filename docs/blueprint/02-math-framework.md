...
**The external rate curve `r(T)` (per-currency risk-free, ingested as an external curve).** This clarifies what the `r(T)` of Eq. 5 *is* once the external rate curve is ingested; it does not change any equation above.
