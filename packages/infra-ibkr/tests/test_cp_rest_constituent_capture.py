...
def _config(
    top_n: int | None, *, capture_pool_size: int = 6, discovery_pool_size: int = 6
) -> PlatformConfig:
...
class _FakeGateway:
    ...
    def _search(self, params: dict[str, Any]) -> Any:
        ...
        if sec_type == "STK":
            assert symbol != "SAN1", (
                "pinned SAN1 conid must come from the pin, never a ticker search"
            )
...
def _capture(store: ParquetStore, top_n: int | None) -> IndexBasket | None:
...
def test_full_membership_scope_captures_every_optionable_constituent(store: ParquetStore) -> None:
    basket = _capture(store, top_n=None)
    assert basket is not None
    underlyings = {k.underlying_symbol for k in basket.instruments}
    # ENEL has no listed options (a clean per-name skip); every other banked member is captured.
    assert underlyings == {"SX5E", "ASML", "TTE", "SIE", "SAN1"}
    by_name = {
        row.underlying: row
        for row in store.read("constituent_capture_outcomes", trade_date=TRADE_DATE)
    }
    assert set(by_name) == set(_WEIGHTS)
...
def test_capture_attempts_all_resolved_constituents_and_records_one_ledger_row_each(
    ...
    partitions = {
        p.parent.name for p in store.root.rglob("constituent_capture_outcomes/**/*.parquet")
    }
...
class _ThrottlingGateway(_FakeGateway):
    ...
    def get(self, path: str, params: dict[str, Any] | None = None) -> Any:
        ...
        if (
            ...
            and int(params.get("conid", 0)) == self._throttle_conid
        ):
            with self._lock:
                self.strike_calls += 1
...
class _ForeignVenueGateway(_FakeGateway):
    ...
    def get(self, path: str, params: dict[str, Any] | None = None) -> Any:
        params = dict(params or {})
        if path == "/iserver/secdef/strikes" and int(params.get("conid", 0)) == self._foreign_conid:
