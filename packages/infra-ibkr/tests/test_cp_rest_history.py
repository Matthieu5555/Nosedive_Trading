...
class _PaginatedGateway:
    ...
    def __init__(self, series: list[dict[str, Any]], *, cap: int = 999) -> None:
        self._series = series
        self._cap = cap
        self.window_starts: list[str | None] = []

    def get(self, path: str, params: dict[str, Any] | None = None) -> Any:
        p = params or {}
        if str(p.get("conid")) == "0":
            return {"data": []}
        start_time = p.get("startTime")
        self.window_starts.append(start_time)
        if start_time is None:
            end_idx = len(self._series) - 1
        else:
...
def test_fetch_pages_backward_over_the_cap_to_full_history(tmp_path: Path) -> None:
    ...
    assert [b.trade_date for b in bars] == [s["date"] for s in series]
    assert len(bars) == 2300
    assert len(gateway.window_starts) >= 3
    assert gateway.window_starts[0] is None
    assert all(start for start in gateway.window_starts[1:])
...
def test_window_cap_bounds_a_nonterminating_feed(tmp_path: Path) -> None:
    ...
    collector.max_history_windows = 2
...
def test_backfill_continues_past_a_failed_ticker(tmp_path: Path) -> None:
    routed = _history_transport(
        ...
        conid_errors={7: _terminal_transport_error(500)},
    )
