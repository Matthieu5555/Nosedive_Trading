...
NOTE — duplicated by construction: this module has a byte-identical twin in the sibling
broker leaf (``algotrading.infra_saxo`` / ``algotrading.infra_deribit``), because sibling
leaves may not import each other (import-linter layer contract). Its intended single home
is ``algotrading.infra.collectors``; hoist it there when that layer is next touched and
delete both copies. A test pins the two copies byte-identical until then.
"""
