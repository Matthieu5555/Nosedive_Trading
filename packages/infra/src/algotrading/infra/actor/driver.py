...
def _option_snapshots_by_underlying_maturity(
    ...
    has no strike/right of its own), keeps only options with a positive maturity AND a
    two-sided quote (:func:`_has_two_sided_option_quote`), and buckets them so a forward
    and a slice can be built per maturity. The inner dict is keyed by option right
    (``"C"``/``"P"``) so the forward pairing is a strike join.
    """
    grouped: dict[
