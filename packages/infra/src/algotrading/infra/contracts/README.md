...
- **Table contracts** — the frozen dataclasses, one per table family from the
  ...
  annualised vega `vega/√T`, ADR 0050) — all raw + cash, additive-nullable.
  ...
  Gamma and vega are not mirrored (put-call parity: identical at one IV, one strike).
  `ScenarioAttribution` is the WS 2C by-Greek decomposition of a
