...
class PricingResult:
    ...
    ``rt_vega`` (running-time / annualised vega, ADR 0049) and its ``dollar_rt_vega`` ride
...
class ProjectedOptionAnalytics:
    ...
    vega ``vega/sqrt(T)``, ADR 0049) are carried per strike in the same dual representation,
    ...
    # RT-Vega (running-time / annualised vega = vega/sqrt(T), ADR 0049) per strike, raw + cash,
