...
RT-Vega (running-time / annualised vega, ADR 0050) monetizes exactly like Vega — it is vega
...
UNIT_STRINGS: dict[str, str] = {
    ...
    # RT-Vega (running-time / annualised vega, ADR 0050): vega/sqrt(T), monetized exactly
...
class DollarGreeks:
    ...
    §7.2), and RT-Vega (running-time vega, ADR 0050). Only the *forked* units are carried as
...
def dollar_rt_vega(rt_vega: float, multiplier: float = 1.0, quantity: float = 1.0) -> float:
    """RT-Vega\\$ = rt_vega·0.01·mult·qty — per 1 vol point (0.01), ADR 0050.
