...
`vol_shocks`, `rate_shocks`, `correlation_shocks`, `named_scenarios`) plus two construction
rules: a combined crash (most adverse spot move paired with the largest vol spike) and a
small time roll-down (`ScenarioConfig.roll_down_days`, Actual/365). The fixed family order is
spot, vol, rate, correlation, combined, time, named. `scenario_line_pnls` produces every
`(scenario, line)` cell (the cartesian product, so completeness is structural); `worst_case`
returns the scenario with the largest portfolio loss plus its lines ranked worst-first.
`scenario_result` projects a full-reprice cell onto `ScenarioResult`.

### Named historical scenarios and the correlation axis (the §5.4 stress screen)

The four §5.4 stress axes are spot, vol, rate, and **correlation**, plus a catalogue of
**named historical scenarios**. The spot/vol/rate axes and the named catalogue all reprice
through the option pricer (`full_reprice_pnl`); the correlation axis reprices through the
basket-variance identity (`correlation_shock_pnl`).

- **Named scenarios** (`ScenarioConfig.named_scenarios`, seeded in `scenarios.yaml` with `2008`
  and `covid-2020`) are each one labelled **compound** shock — a joint spot/vol/rate move
  applied as a single full-reprice scenario (`family="named"`, id `named_<label>`), expressing
  a whole crisis day at once rather than the parametric grid's one-axis-at-a-time sweep. The
  shipped magnitudes are **owner-tunable seeds** sized to the broad shape of each crisis (a
  large spot crash + a big vol spike + a rate cut), **not** calibrated to a specific source —
  tune them to the course/blueprint reference when the owner sets the bar.
- **Correlation shocks** (`ScenarioConfig.correlation_shocks`, empty by default) bump the
  basket's average implied correlation ρ̄ and reprice the basket vol through the Eq-23 identity
  (`basket_variance`). `BasketCorrelationExposure` carries the basket (weights, vols, base ρ̄)
  plus the dollar basket-vega the book holds against ρ̄; `correlation_shock_pnl` recomputes the
  basket vol under the bumped ρ̄ (a genuine reprice, never a Taylor multiplier) and monetizes
  the basket-vol change. **This family is built but dormant**: a standard option line carries no
  ρ̄ exposure, so a ρ̄ bump reprices to zero through the pricer. It becomes meaningful only once
  a real ρ̄ exposure (constituent capture + the signal layer, both downstream) lands; until then
  it is exercised only against a constructed basket exposure in tests. A ρ̄ bump that drives the
  basket non-PSD surfaces `NonPSDBasketError` rather than silently flooring.

Each new family folds into the construction hash **only when non-empty**, so an unconfigured
grid (no rate / no correlation / no named family) is byte-identical to before those families
existed — the single invariant they are built around (the rate-axis precedent). The
`scenarios` config-bundle hash *does* move when the fields are added (a bundle hash sees the
new keys, empty or not); that golden is regenerated by design. ADR 0049 records why the named
family composes as a compound pricer reprice while the correlation family reprices through a
second path (`basket_variance`), and why the latter is built but dormant.
