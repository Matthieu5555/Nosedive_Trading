...
class SnapshotMarketState:
    ...
    ``spot`` is the underlying reference spot; ``discount_factor_at`` maps a maturity (in
    years) to its discount factor so each tenor cell discounts correctly. Carry is taken as
    zero (the spot==forward, Black-76/futures view) so spot and forward delta coincide,
    matching the delta-band inversion convention; the forward at a tenor is then
    ``spot * discount-free`` — i.e. the forward equals spot here, and the discount factor
    only scales the option price. This is the same ``carry == 0`` pin the 1B delta-band
    selection uses.
    """
    ...
    def discount_factor_at(self, maturity_years: float) -> float:
        """The discount factor for a maturity, falling back to the default."""
        return self.discount_factors.get(maturity_years, self.default_discount_factor)
...
def project_grid(
    ...
    *,
    ...
    for tenor_label in projection.tenor_grid:
        ...
        discount_factor = market.discount_factor_at(maturity)
