...
:class:`~algotrading.infra.validation.state.ValidationCheck` so the run rolls up into one
:class:`~algotrading.infra.validation.state.ValidationReport`.
...
# anomaly flags without parsing the detail string. The triage layer also keys the
# ``source="anomaly"`` discriminant off this code.
...
def run_validation(
    *,
    ...
    :func:`~algotrading.infra.validation.anomaly.detect_anomalies`), so a stored report is
    deterministic regardless of metric insertion order.
    """
