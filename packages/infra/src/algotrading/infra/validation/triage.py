...
Two planes flag problems with a day's run — the named QC checks
(:mod:`algotrading.infra.qc`) and this validation plane's anomaly detection. Before a
reporting or alerting layer can act, those two result shapes must collapse into one, or
every consumer has to reconcile them. This module is that collapse: it folds a
:class:`~algotrading.infra.qc.QcReport` and a
:class:`~algotrading.infra.validation.engine.ValidationOutcome` into one worst-first list
of :class:`~algotrading.infra.contracts.TriageRecord` rows, and maps the list to one
escalation level.

**One persisted shape, three sources.** Every row is a ``TriageRecord`` in the single
``triage_records`` table, discriminated by ``source``: ``"qc"`` for a named static check,
``"anomaly"`` for a rolling-baseline metric flag, and ``"validation"`` for any other
validation check. The anomaly/validation split is read off the check's ``reason_code``
(``REASON_METRIC_ANOMALY`` ⇒ ``"anomaly"``), so the discriminant lives in one rule and
the data dictionary's three-source promise is real, not nominal (ADR 0010, C2).

The merge preserves both planes' discipline. The specificity rule survives — a QC row's
headline is built by the *same* :func:`algotrading.infra.qc.result_headline` an operator
already reads, and its offender name by the same logic — so "surface fit failed for AMS
2026-09" does not decay into "QC red" on its way into the unified table. The escalation
rule is one policy in one place: a critical-severity failure pages, any other failure or
warning is a notice, a clean run escalates to nothing.

The records are pure values. Persisting them is the caller's job (the orchestration layer
writes them to the ``triage_records`` table through ``StorageRepository``); this layer
never does I/O.
"""
...
def _validation_source(check: ValidationCheck) -> str:
    """Discriminate a validation check's triage source.

    A metric-anomaly flag (``reason_code == REASON_METRIC_ANOMALY``) is the rolling-
    baseline plane proper, so it lands under ``source="anomaly"``. Any other validation
    check — a structural or cross-field validation that is not an anomaly score — lands
    under ``source="validation"``. This is the single place the two sources part, so the
    ``triage_records`` discriminant cannot drift from the reason a check declared.
    """
...
def triage_from_validation(outcome: ValidationOutcome) -> tuple[TriageRecord, ...]:
    """Fold a validation outcome's flags into unified records.

    Each non-passing check becomes a row whose ``source`` is ``"anomaly"`` for a metric-
    anomaly flag, else ``"validation"`` (see :func:`_validation_source`). A
    ``NO_BASELINE`` metric maps to a PASS check — nothing to act on yet — so it is left
    out; the triage list is things to investigate, not things to await.
    """
