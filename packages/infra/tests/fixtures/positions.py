"""Named position and valuation fixtures for the risk engine (M3).

Risk extends the shared fixture library (per ``tasks/TESTING.md``): the risk edge-case
...
independent oracles were derived against (see ``test_risk.py`` provenance comments):
spot 100, carry 0 (so forward 100), T 0.25, vol 0.20, DF 0.99, multiplier 100, USD,
European — chosen so the at-the-money call price is the familiar 3.947884 anchor.

The "rogues' gallery" entries here are the ones the risk edge cases need: a
low-confidence contract (the quote was flagged) and a non-USD contract for
