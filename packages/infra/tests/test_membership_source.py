...
# A committed-CSV seed with a weight column — the MVP path for an index whose free feed has no
# weights. Hand-authored; the expected weights are read straight off these literals.
...
def test_csv_file_source_carries_weights(tmp_path: Path) -> None:
    ...
    assert changes["CCC"].weight is None  # blank cell → labeled unavailable, never zeroed
    for change in changes.values():
        assert change.index == "SX5E"
        assert change.vendor == "seed:test-snapshot-2026-06-10"
        assert change.effective_add_date == date(2000, 1, 1)  # no add-date column → fallback
