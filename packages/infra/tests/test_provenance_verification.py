...
Relocated from ``backend/tests`` onto the merged ``packages/`` stack (C3): it drives
``algotrading.infra.actor.run_day`` and validates with
``algotrading.core.provenance``, so the provenance invariant is now proven inside the
root gate on the real merged code, not on the dead flat tree.
"""
...
from algotrading.infra.actor import run_day
