...
def _select_basket_source(store: ParquetStore) -> BasketSource | None:
    ...
    ``store`` is threaded into the source so the capture widens to the index's full point-in-time
    membership of constituent option chains (ADR 0059, re-enabling the lane ADR 0051 retired): the
    source reads the as-of membership weights from it. It is the same store the runner persists into
    (the canonical ``data_root``), so the membership the source reads is the membership the platform
    banked. This is additive to the dispersion ρ̄ diagnostic, which is unchanged (realized
    constituent vol over the full membership). Scope is config-gated
    (universe.capture_constituents).
    """
