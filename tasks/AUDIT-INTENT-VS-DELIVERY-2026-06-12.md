...
| **QC-3** | `qc.yaml:37 max_delta_step 0.25`; `qc/checks.py:660-735` | the band must be densely covered (the prof wants the ±30Δ band by **steps of 2**: 30/28/…/02/ATM/02/…/30) | a `0.25` max step is **half the full ±0.30 band** — a tenor with only {−0.30,+0.30} (no interior) could nearly pass the step test; AND the band itself was emitted at **step-10, 8 points** from a `.py` literal (`projection.py _DEFAULT_BAND_*`), with no YAML supplying it and the QC spanning the *realized* delta (ATM≈±0.5) so the pas-2 step was unenforceable. The economic grid the prof drew was overwritten by a technical default, gate green. **4th confirmed instance of this class** (after Cap-1 tenor, Cap-2 delta-window, An-3 surface-arbfree) | tests feed compliant/obviously-broken delta sets at the loose 0.25 step | self-consistent but **loose**, and the band shape itself was wrong | **MED** | **RESOLVED — `T-delta-step-2` (2026-06-13):** band out of `.py` into typed config (`GridQcConfig.band_step`, `ProjectionConfig.from_band`), `max_delta_step 0.25→0.02`, QC spans `target_delta` so it **forces** the pas-2 grid; one band def feeds both emit + validate. `atm`+`atmp` kept. Goldens regen by design (pre-capture). |
