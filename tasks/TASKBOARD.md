...
| Matthieu (Claude) | C7 config hardening — **landed** (clearing claim) | 2026-06-06 | Inc 1–5 done: six Part VII YAMLs + bundle loader; every hashed economic param (roll_down / solver-bracket / SVI / forward heuristics) in typed config; per-bundle `config_hashes` dict on every `ProvenanceStamp`; injected code identity (SHA+dirty) on the run manifest; per-run config freeze + `validate_manifest`. **Replay-a-run reproducibility is locked — the owner gate for new compute is reopened.** Two explicitly-staged carry-forwards remain (see C7 spec): the effective-dated profile *store* (ADR 0028 "Next" stage — for replaying a *past day* fresh) and operational broker.yaml client-id bands (not hashed). Gate green. |
