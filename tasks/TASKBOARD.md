...
| Claude (cockpit-ux) | spec + sequential batches across `apps/frontend/web` + `configs/scenarios.yaml` (see [frontend-cockpit-ux-overhaul](frontend-cockpit-ux-overhaul.md)) | 2026-06-17 | Cockpit UX/visual overhaul (owner-ratified A–E). ① (charts/CSS) + ② (Orders) **landed**. ⚠️ **Batch ③ + E + greeks-table sci-notation touch `DollarGreeksByMaturity.tsx`/`Market.tsx`/`api.ts` — SERIALIZE behind the active `frontend-second-order-greeks-panels` lane above** (rebase onto its land first; do not edit those files concurrently). |
