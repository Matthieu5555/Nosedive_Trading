...
| Claude (agent) | docs/ADRs/tasks only — NEW: `.agent/decisions/0028-*`, `tasks/{C7-config-hardening,C8-hygiene-fixes}.md`; EDITED: `documentation/configuration-and-reproducibility.md`, `.agent/open-questions.md`, ADR-notes on `.agent/decisions/{0018,0003,0015,0016}`, `tasks/{C5-retire-backend,ibkr-rest-api-evaluation,archive/README}.md` | 2026-06-05 | **Done — post-audit ratify + triage pass (no code).** (1) **Config standard revised after a 3-expert red-team** (caught: doc described a loader that doesn't exist — two non-composable config paths; `composite_config_hash` diverged from the blueprint's **per-bundle** `config_hashes` manifest; real repro holes int/float, NaN/-0.0, completeness-by-discipline, `code_version` defeated by dirty tree). (2) **ADR 0028 ratifies it**; resolves **OQ-5** (keep storage port load-bearing) + **OQ-6** (profiles = effective-dated + content-addressed runtime store + per-run manifest freeze; **config is as-of like market-data/membership** — replays a past day; git is dev-time only). (3) **C7** = the 5 config fix-tasks (seq after C6 for the wiring); **C8** = orthogonal hygiene (valuation_join + quote_quality tests, the Deribit `discover_instruments` wall-clock determinism leak; **C8-A frontend reds already fixed by C4**). (4) **ADR status-notes** added to 0018 §3 (pull `BrokerSession` retired by 0027), 0003 (pull mechanics→0027, idempotency kept), 0015 (M10 landed; port kept per OQ-5), 0016 (EventSource YAGNI under Nautilus). (5) **Task truth:** C5 spec + archive/README now reflect C6; ibkr-eval marked ADR 0024 **accepted+landed**. **HANDOFF to doc-pass owner (you hold these files):** add C1-spec note that its broker-unification/idempotency items moved to **C6** (ADR 0027); extend **ADR 0008**'s status note to point at 0024/0025 (REST landed), and the cosmetic 0024 "Proposed"→accepted / 0025 0024-ref fix; register **C7/C8** in the workstreams area. **FLAG:** **C3 is substantial-but-uncommitted** (board row says so) — commit it before any C5 deletion so a red gate ≠ a deletion regression. |
| Claude (agent, for Vincent) | merge→`main` + branch cleanup; **C5** `backend/**` deletion + doc truth (`.agent/{map,glossary}`, `AGENTS.md`, `README.md`, `documentation/**`, `pyproject.toml`); Test Lenny exclude | 2026-06-05 | **DONE on `main`.** Consolidated C1–C6 onto `main` (fast-forward) and deleted 12 stale branches. **C5 landed (`c416163`):** removed the whole `backend/` flat tree; repointed the 18 `documentation/modules/` mirror symlinks + every runbook/doc/README/map/AGENTS/glossary ref off `backend/src` onto `packages/infra`; dropped the now-no-op backend tooling-excludes. Root gate was green in isolation immediately before the collision below. **⚠ Vincent's Code removal BLOCKED (step 8):** the dir is owned by `matthieu`, this process runs as `vincent` → `rm` is permission-denied (clone **intact, undamaged**); needs matthieu/sudo, after which I drop its pyproject/.gitignore excludes. **⚠ LIVE COLLISION:** `apps/frontend/src/algotrading/frontend/runner.py` is under concurrent edit (the C6 SAMPLE live-run build) — I did **not** touch it; the current root-gate red is entirely that in-flight file, not C5. Runbook steps 9–11 (minimalism/contract-tests/C7) + C8 **paused** pending owner go-ahead — they touch shared `packages/infra` under active multi-agent editing and need a green baseline. |
