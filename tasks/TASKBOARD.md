...
| Matthieu (Claude) | index-registry design (doc-only): `.agent/decisions/0035`, `tasks/1J-index-registry.md`, edits to `tasks/{1A,1C,1G}`, `.agent/open-questions.md` (OQ-8/9), `documentation/{roadmap-index-analytics,connectivity/server-deployment-plan}.md`, TASKBOARD | 2026-06-07 | Adds the **index registry** (which indices the scheduler fetches + IBKR contract ref + per-index exchange calendar) to `universe.yaml`; closes the run-window open decision via exchange-calendar library (ADR 0035). **No code** — config/spec only; `configs/universe.yaml` edit deferred to the 1J impl. |
...
| **Phase 1 (Tab 1)** | Per-WS specs written: [1J index registry](1J-index-registry.md) (foundational — which indices to fetch + per-index exchange calendar, [ADR 0035](../.agent/decisions/0035-index-registry-and-per-index-capture-schedule.md)) → [1A membership](1A-universe-membership.md) → [1B Δ-band](1B-delta-band-selection.md) → [1C capture](1C-capture-daily-close-and-history.md) → [1F projection](1F-analytics-projection.md) → [1G cron](1G-cron-daily-close.md) → [1H QC](1H-qc-index-grid.md) → [1I front+API](1I-front-page.md). Futures gated/parallel: [1D](1D-futures-term-structure.md). 1E folded into P0/1C. | per-WS (this dir) |
