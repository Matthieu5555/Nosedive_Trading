...
| Claude (strategy tech-lead) | new `packages/infra/src/algotrading/infra/risk/decorrelation.py` + tests; additive `decorrelation` block on the BFF `/api/compose` payload (`routers/compose.py`) (see [strategy-decorrelation-analytics](strategy-decorrelation-analytics.md)) | 2026-06-17 | §5.8 decorrelation **verification** (post-week): read-only cross-strategy stressed-P&L correlation + shared-tail overlap + factor overlap + marginal-contribution-to-risk over the landed 2D book. **Realized-series correlation + marginal Sharpe are GATED honestly — no banked per-layer realized P&L on the live book, no fabricated ρ̄.** No optimiser. No `apps/frontend/web`. |
| Claude (reconstruct-signals-qc) | `packages/infra/src/algotrading/infra/orchestration/reconstruction/batch.py` + `packages/infra/tests/test_replay_reconstruction.py` (+ delivery assertion in `packages/infra-ibkr/tests/test_real_sample_reconstruct.py`) | 2026-06-17 | T-clean-ingestion §0 steps 2-3: extend `reconstruct_day` to also run `persist_signal_set` (ρ̄) + `run_qc`/triage on recompute-from-raw (Part-XV complete), behind `persist=True`. Mirrors live eod `_analytics`/`_qc`. |
