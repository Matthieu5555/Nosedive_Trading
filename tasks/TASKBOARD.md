...
| claude (OHLC constituent backfill) | `infra-ibkr/collectors/cp_rest_history.py`, `infra-ibkr/history_backfill.py`, `infra/storage/adapter.py` (read-only helper), `scripts/ohlc_backfill.py` + their tests | 2026-06-12 | [ohlc-constituent-backfill](ohlc-constituent-backfill.md): fix the per-ticker full-table presence scan (O(N²) — the real stall, beyond the diagnosed warmup/503), expose `--max-windows`, then RUN for SX5E+SPX (gateway is up+authed). Bounded depth (1y) to not worsen the daily-bar file blow-up before compaction. |
