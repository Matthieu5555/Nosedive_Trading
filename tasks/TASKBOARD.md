...
| 📋 SPEC READY — coverage panel | **[T-capture-coverage-panel](T-capture-coverage-panel.md)** — per-expiry capture table + per-tenor QC coverage on Tab 1, from already-persisted `instrument_master` + `qc_results` (no recompute). Makes data-quality / term-structure gaps visible at a glance (serves review-priority #1). New files `routers/coverage.py`, `components/CoverageTable.tsx` (disjoint) — but **the `App.tsx`/`Market.tsx` placement edit overlaps the front lane (codex): coordinate**. | 2026-06-12 | not started; spec written |
| 📋 SPEC READY — delta window | **[T-delta-window](T-delta-window.md)** — `_DISCOVERY_STRIKES_PER_SIDE=16` (±~1%) **clips the 30Δ band** the prof asked for (verified: 30Δ at +99 pts/10d → +1370/3y vs ±78 captured; `delta_band_completeness` QC=FAIL). Make the discovery strike window delta-driven + tenor-aware. **Same files/functions as the landed T-tenor-selection** (`cp_rest_close_capture.py` `_discover_chain`/`_nearest_strikes`) — rebase on `74d2cc7`. Owner decision in-spec: full-30Δ vs capped+labeled (pacing). | 2026-06-12 | not started; spec written. Urgent — pairs with tenor fix (band widest at long tenors). |
...
| claude (OHLC constituent backfill) | `infra-ibkr/collectors/{cp_rest_history,cp_rest_discovery}.py`, `infra/storage/adapter.py`, `scripts/ohlc_backfill.py`, `frontend/__main__.py` + tests | 2026-06-12 | [ohlc-constituent-backfill](ohlc-constituent-backfill.md) **RAN**: 548/553 underlyings have 1y bars (`4bd6536` one-pass presence scan + `--max-windows`; venue-aware conid resolution — blind first-match resolved 'DTE'→DTE Energy, 'AIR'→AAR Corp, etc.). **OPEN: owner go needed to purge+refetch the 70 divergent names** (wrong-company bars on disk from the first pass). `NDA FI` = HEX data-permission gap (labeled). **BFF wedged** (uvicorn reload watcher recursing into `data/`; launcher fixed) → needs a restart by `vincent`. |
