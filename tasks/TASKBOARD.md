...
> 4. **Risk / stress / scenarios = ONE agent at a time.** ✅ **Largely resolved 2026-06-17 (Stream B):**
>    `frontend-scenario-rate-axis-wiring` + `strategy-composition` **landed & archived**, and the
>    Onglet-2 ④ Attribution half of `frontend-second-order-greeks-panels` landed — all through a single
>    front-risk-lane integrator (`b2f95bb`), no race. The rule still stands for the **Onglet-1 ③ Panneau
>    Ténor** 2nd-order remainder (touches `TenorPanel.tsx` on the Onglet-1 lane). The PnL/stress compute
>    substrate was already landed; these were front/BFF wiring slices over it.
...
| A-tech-lead stream-A | `routers/analytics.py` (`_nearest_quote` strike-match tolerance) + `apps/frontend/tests/test_analytics_api.py` + board hygiene | 2026-06-17 | Stream-A closeout: design-review fix (off-grid quote bind) + retire/mark the landed fiches; shell flip NOT touched |
...
- ~~[execution-operational-hardening](execution-operational-hardening.md)~~ **ALL FOUR SUB-LANES LANDED 2026-06-17** (§7.9 umbrella): margin/InvWC capacity (C3, `b231924`), kill switch (C2, `11006d3`), broker recon (prior, `execution-recon-diff`), alert delivery (C4, `d944b41` — the shared `AlertSink` seam). Umbrella complete; archive candidate.
