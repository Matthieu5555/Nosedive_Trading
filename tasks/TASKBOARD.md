...
| claude (frontend-risk-scenarios-rework) | [frontend-named-scenarios-wiring](frontend-named-scenarios-wiring.md) (+ recon/attribution) — branch `frontend-risk-scenarios-rework`: reworked `apps/frontend/web/src/pages/RiskScenarios.tsx`, new `src/components/{NamedScenarios,Reconciliation}.tsx` + tests, additive `src/api.ts` recon types + `fetchReconciliation`, `src/stressApi.ts` `NamedScenario`, `src/hooks/queries.ts` book-attribution + recon hooks, new `e2e/risk-scenarios.spec.ts`; additive BFF `named_scenarios_to_list` + `/api/risk/scenarios.named` (serializers.py + routers/risk.py) + tests | 2026-06-16 | **landed** — page is meaningful on first load: named historical scenarios (worst-first), book-level P&L attribution waterfall (`/api/attribution?level=book`), broker reconciliation (`/api/reconciliation`, plain "does the broker agree?"), plus the kept persisted surface; portfolio + broker-account selectors; no-broker-account 400 is a labelled empty state. Named families surfaced; **correlation family stays dormant** (no real ρ̄ exposure on the live book — left as the spec's follow-up). On-demand book-level stress is NOT built (no backend endpoint; the Basket "Stress basket" remains the on-demand path) — deferred. Gate: web lint clean, 195 vitest, 188 BFF pytest, e2e 50 pass / 1 pre-existing Market failure (`pages.spec.ts:9`); ruff+mypy+lint-imports clean. |
