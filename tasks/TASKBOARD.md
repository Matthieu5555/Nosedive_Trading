...
| 🚧 vincent+claude (coverage panel) | **[T-capture-coverage-panel](T-capture-coverage-panel.md)** — BFF: **new `routers/coverage.py`** + `frontend/app.py` (register only). Front: **new `components/CoverageTable.tsx`** + later the `Market.tsx`/`App.tsx` placement edit (**overlaps codex front lane — coordinate that one edit**). Reads already-persisted `instrument_master` + `qc_results`, no recompute. | 2026-06-12 | BFF endpoint in progress |
