...
| Claude (agent) | `packages/infra/src/.../storage/{ports,runs,sqlite_runs,postgres_runs,factory}.py`, `packages/infra/tests/test_run_repository.py`, `packages/infra/pyproject.toml` | feat/merge-postgres | 2026-06-05 | M10: Postgres serving tier — RunRepository port + SQLite reference + PostgresRunRepository + factory (POSTGRES_URL toggle) + conformance tests. |
