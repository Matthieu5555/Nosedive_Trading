...
> [!IMPORTANT]
> **▶ SCOPE — index-options-only ([ADR 0042](../.agent/decisions/0042-index-options-only-scope-ibkr-sole-broker.md); T-index-only-refactor LANDED 2026-06-13). READ FIRST.**
> The app is **EuroStoxx-50-first index options**. **IBKR is the sole live broker** (Saxo + Deribit
> packages removed); **SX5E is the single live index** (SPX parked `enabled:false`); single names are
> index *constituents*, **never option underlyings**. If any spec, ADR, README, or doc still says
> "Saxo", "Deribit", "three brokers", or "equity underlying", **the index-only pivot wins** — do not
> resurrect that code. The affected ADRs carry a dated `> **AMENDED 2026-06-13**` note at their top
> (**0012 / 0017 / 0022 / 0023 / 0025 / 0026 / 0027 / 0035**; **0013 / 0014 superseded**) — read the
> note, not the original multi-broker body. Six specs were patched to IBKR-only the same day:
> **3A, 3B, security-review, REP7, server-deploy-plumbing, T-pricing-config-completeness**.
> **1D-futures stays parked** (ADR 0037 forward-only; index-only does not re-open it).
>
> **▶ INCOMING AGENTS — ready queue (no rescope needed, disjoint, pick one and claim a row):**
> audit remediation → **T-qc-residual-units** (owner #1), **T-surface-arbfree-qc**,
> **T-scenario-rate-axis**, **T-strike-window-pct-clip**; capture correctness →
> **clock-timer-coherence** (the SX5E/XEUR timer — the *live* one; the "XEUR bug" is SX5E's, not a
> parked index), **daily-bar-compaction** (971k `daily_bar` files on disk — bloat is real regardless
> of SPX parking); front → **T-capture-coverage-panel** (only the `Market.tsx` placement left),
> **front-page1-cdc-buildout**; Phase 2 → **2C-pnl-attribution**, **2D-strategy-composition**;
> library-leverage → **REP0/REP2/REP3/REP4/REP5/REP6/REP9/REP10**; **ci-pipeline**.
> **Patched-then-ready (IBKR-only now):** 3A, 3B, security-review, server-deploy-plumbing, T-pricing.
> **Blocked/parked (do NOT start):** 1D-futures (parked), REP7 (needs a live `TradingNode`),
> REP8 (IBKR live-auth), REP1 (won't-fix), T-raw-invariant (after QA-FIX).
>
...
> arb flags the vol-surface lane propagated). Latent mines: [T-capture-config-coherence](archive/T-capture-config-coherence.md),
...
| 🟢 Claude (vincent) — delta-step-2 grid LANDED (gate green) | **[T-delta-step-2](archive/T-delta-step-2.md)** (audit An-6 residual) — ±30Δ projection band at **step 2** (15 puts + atm + atmp + 15 calls = 32/tenor) out of `.py` literals into typed config (ADR-0028). `surfaces/projection.py` (`delta_band_axis`/`from_band`), `actor/driver.py` (axes from `qc_threshold.grid`), `core/config/platform_config.py` (+`band_step`), `configs/qc.yaml` (`band_step:0.02`, `max_delta_step 0.25→0.02`), `qc/inputs.py`+`qc/checks.py` (band check spans **target_delta**), `surfaces/README.md`, tests. Projection+qc config-hash goldens regen **by design** (pre-capture). **Gate: ruff ✓ mypy ✓ (210) lint-imports ✓ pytest 1390/3** (3 = pre-existing `documentation/` deletion). | 2026-06-13 | Owner-gated (prof = pas-2). `atm`+`atmp` **kept** (front straddle). **Needs a re-run/re-capture** to fill the 31/32 points. |
| 🔵 Claude (vincent) — IN PROGRESS (front fix) | **[T-front-currency-and-bands](T-front-currency-and-bands.md)** — page-1 validated, **fix only**: (a) monetized Greeks in **€** from the registry currency (blueprint 05-math-notes "correct currency"), **backend single-source LANDED** (`/api/indices` now serves `currency` from `IndexEntry.currency`; `routers/indices.py`+`tests/test_indices_api.py`); front display wiring (`api.ts`, `DollarGreeks.tsx`, `MaturityAccordion`, `format.ts`) remains. (b) un-hardcode `BasketLegGrid` band list → backend band-axis source (32 bands, scroll). | 2026-06-13 | Disjoint from the landed `/api/indices` registry lane; explicit-path staging. |
| 📋 SPEC READY (owner-asked, blueprint-settled) | **[T-explicit-rate-parameter](T-explicit-rate-parameter.md)** — make the IR an **explicit, typed-config, displayed, modifiable** input (currently implicit `−ln(DF)/T`, an ADR-0028 gap). Blueprint pins the design (Eq 2/Eq 5/Step 6f/impl-guide `estimate_forward(…rate…)` + `forward_curve` carry diagnostic) — no owner decision needed. Folds with **T-scenario-rate-axis** (rate shocks). | 2026-06-13 | rho is the rate Greek; already shown. This makes the rate behind it explicit + tunable. |
...
> (rulings recorded, ready) + **[ohlc-constituent-backfill](archive/ohlc-constituent-backfill.md)**; (4)
...
> 3. **Constituent Charts:** Launch the [ohlc-constituent-backfill](archive/ohlc-constituent-backfill.md) so constituent candlestick charts are not empty.
...
| **Phase 1 (Tab 1)** — ✅ **1A–1J done (gate-green)** | Shipped: 1J index registry → 1A membership → 1B Δ-band → 1C capture → 1F projection → 1G cron → 1H QC → 1I front+API, plus the P0 contracts and the D1 `provider` partition foundation. All specs archived to [`archive/`](archive/). **PARKED, do not start: [1D futures term-structure](1D-futures-term-structure.md)** — blocked on P0.4 (futures deferred forward-only, ADR 0037); the index-only pivot does **not** re-open it. No-op until a future ADR amends the blueprint with a futures product. 1E folded into P0/1C. | per-WS → [archive/](archive/) |
...
- **[OHLC constituent backfill](archive/ohlc-constituent-backfill.md) — queued (2026-06-10), unclaimed.** Per-component
