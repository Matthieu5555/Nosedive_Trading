...
| Claude (matthieu) | `packages/infra-ibkr/**` constituent option capture (`collectors/cp_rest_close_capture.py`, new `collectors/cp_rest_constituent_capture.py`, `live_capture.py`) + `core` `UniverseConfig.constituent_top_n` + `configs/universe.yaml` | 2026-06-14 | [ibkr-constituent-option-capture](ibkr-constituent-option-capture.md) — widen capture to top-N constituents; membership top-N seam stubbed pending [infra-sx5e-weighted-membership](infra-sx5e-weighted-membership.md) merge (isolated worktree branch) |
