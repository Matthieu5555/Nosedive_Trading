...
### OWNER RULING (2026-06-12) — implemented

The owner **reversed the spec's recommendation** after the day's intent-vs-delivery lesson:

- **Pacing → full 30Δ, NO cap.** A generous per-tenor strike cap is the *same* intent-vs-delivery
  bound this task removed, just relabelled — a knob someone can mis-set that re-clips the 30Δ. So
  there is no cap. Full-30Δ is bounded in practice by the broker's listed strikes (coarse spacing
  at the long end → tens of strikes, not hundreds; ~400–700 `secdef/info` total over 8 tenors,
  minutes on a daily EOD job with the 429 backoff already in place). The only backstop is a
  **fail-loud runaway guard** (`DiscoveryRunawayError`, far above any real SPX/SX5E expiry) — it
  *raises*, never silently trims; a pathology valve, not a cap that bites.
- **Working vol → single conservative seed in config**, not per-index (index-addition coherence:
  adding an index stays "conid + enabled", no per-index value to hand-tune that drifts). Set
  conservative-HIGH so we over-qualify, never under (under-sizing re-creates the clip). It is a
  request-shaping *sizing* seed, not the economic band selector (the band is chosen downstream
  with the fitted vol). If a genuinely more volatile index ever clips, `delta_band_completeness`
  QC turns red and an override is added *then*, data-driven — never speculatively now.

**Implementation (landed on `audit-fixes-batch1`, gate green 1404/16):**
- `core.config.StrikeSelectionConfig.discovery_working_vol` (default 0.40) + `configs/universe.yaml`
  — the conservative seed; folds into the universe config-hash (golden regenerated by design,
  ADR 0028 / C7 pattern; pre-capture dev change, no banked record carries the old hash).
- `universe.chain_planning.discovery_delta_bound` (economic bound − ~20Δ margin) +
  `select_discovery_strikes` — the broker-neutral discovery window. It **reuses
  `select_strikes_delta_band`** (the same single delta source, the pricing engine) at the looser
  bound + seed vol, so the window scales with √T and is a guaranteed superset of the 30Δ band.
- `cp_rest_close_capture._qualify_strikes_for_expiry` replaces `_nearest_strikes(…, 16)` per
  expiry; `_DISCOVERY_STRIKES_PER_SIDE` is gone (a flat count cannot bound a √T-growing band).
  `_nearest_strikes` survives only as the no-spot fallback; `DiscoveryRunawayError` is the valve.
- Tests: independent `scipy.norm.ppf` oracle vs the engine, "economic selection over the discovery
  window == over the full ladder" (superset proof), tenor-aware widening, short-tenor stays tight,
  garbage-vol labeled raise, runaway raise, determinism. Look-ahead audit clean.
