...
> **LANDED 2026-06-16 — commit `b128d6b`.** One shared `BoundedSemaphore`
> (`StrikeSelectionConfig.capture_pool_size`, default 6) gates every gateway `.get`: the
> cross-underlying fan-out and the within-underlying `/secdef/info` walk both draw from it, so
> `discovery_pool_size` composes into the budget rather than multiplying it (the nested-pool
> 429-storm hazard the spec warned against). Output is byte-identical to the serial path at any
> width (`capture_pool_size=1` reproduces it), locked by a serial-vs-concurrent parity test and a
> concurrency-ceiling test. Gate green. **One demonstration remains outstanding:** done-criterion
> #1's *measured, recorded* live wall-clock margin inside the post-close settlement window — the
> mechanism + tests landed, but the live-capture margin has not been observed and recorded. Archived
> under the same merged-on-main bar that retired its parent [EMERGENCY-capture-throughput](EMERGENCY-capture-throughput.md).
