...
> **SUPERSEDED — archived 2026-06-14.** This slice was built around a TWS-**socket** smoke
> test (`scripts/ibkr_bootstrap.py` driving `IbkrTransport`). Ruling **R4** (TARGET §4) made
> the **CP-REST** gateway the sole IBKR path and parked the TWS socket, so `ibkr_bootstrap.py`
> was never created and is no longer the right artifact. The deploy *intent* — "confirm a
> headless gateway answers before the gated capture runs" — is satisfied in reality by the
> CP-REST stack that landed instead: `scripts/ibkr_gateway_login.py` (headless login + SMS
> 2FA), `scripts/eod_babysitter.py` (keepalive + per-index close fire), and the systemd units
> under `scripts/systemd/` (ADR 0032). `.env.example` at the repo root exists. That real
> deployment stack now has its own governing spec —
> [platform-deploy-stack-ownership](../platform-deploy-stack-ownership.md) — which also carries
> the still-deferred `docker-compose` + supervised-collector pieces this slice listed as
> out-of-scope. Body kept below for history.
